    ...
    db.collection("products").orderBy("kategori").onSnapshot(s => {
        ...
        if (currentUser && currentUser.role === 'admin') {
            document.getElementById("adminCatalogTable").innerHTML = catalog.map(p => {
                ...
                const stock = getStock(p);
                const stockLabel = habis ? `<span style="color:#c0392b;font-weight:bold;">${stock} (Habis)</span>` : (stock <= 9 ? `<span style="color:#c9772a;font-weight:bold;">${stock} (Menipis)</span>` : `<b>${stock}</b>`);
