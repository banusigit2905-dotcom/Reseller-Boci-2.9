...
function loadAdminData() {
    ...
    db.collection("orders").onSnapshot(snap => {
        ...
        document.getElementById("adminOrderTable").innerHTML = filtered.map(d => {
            ...
            return `<tr><td><b>${o.resellerName}</b></td><td>${o.customerName}</td><td>${o.produk}</td><td>Rp ${o.total.toLocaleString('id-ID')}</td><td>${o.status==='pending'?`<button onclick="updateStat('orders','${d.id}')" style="background:#F2A93B; border:none; padding:5px 10px; border-radius:5px; color:white; font-weight:bold; cursor:pointer;">Selesai</button>`:'✅'}</td></tr>`;
        }).join('') || '<tr><td colspan="5" style="text-align:center; padding:20px;">Data tidak ditemukan</td></tr>';
    ...
    // Data lainnya (Return, Complaint, Redeem)
    db.collection("returns").onSnapshot(s => {
        ...
        document.getElementById("adminReturnTable").innerHTML = s.docs.map(d => `<tr><td><b>${d.data().nama}</b></td><td>${d.data().produk}</td><td><button onclick="updateStat('returns','${d.id}')">Selesai</button></td></tr>`).join('');
    });
    db.collection("complaints").onSnapshot(s => {
        ...
        document.getElementById("adminCompTable").innerHTML = s.docs.map(d => `<tr><td><b>${d.data().nama}</b></td><td>${d.data().pesan}</td><td><button onclick="updateStat('complaints','${d.id}')">Selesai</button></td></tr>`).join('');
    });
...
function changeRankPage(dir) { currentRankPage += dir; renderRankTable(); }

// --- LOGIKA ADMIN TAMBAHAN ---
function loadActivationList() {
    db.collection("users").where("role", "==", "reseller").where("isActive", "==", false).onSnapshot(snap => {
        document.getElementById("adminActivationTable").innerHTML = snap.docs.map(doc => {
            const u = doc.data();
            return `<tr><td><b>${u.customId}</b></td><td>${u.nama}</td><td>${u.email}</td><td><button onclick="activateUser('${doc.id}')" style="background:#4A633C; color:white; border:none; padding:5px 10px; border-radius:4px; cursor:pointer;">AKTIFKAN</button></td></tr>`;
        }).join('');
    });
}
