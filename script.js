...
function renderProductList() {
    ...
    listEl.innerHTML = list.map(p => {
        ...
        const stockClass = !hasLimit ? '' : (habis ? 'stock-out' : (p.stock <= 9 ? 'stock-low' : 'stock-ok'));
        const stockLabel = !hasLimit ? '' : (habis ? 'Habis' : `Stok ${p.stock}`);
        ...
        return `<div class="p-row ${habis ? 'disabled' : ''}">
                    <div class="p-info">
                        ...
                        <div class="p-meta">
                            ...
                            ${hasLimit ? `<span class="p-dot">•</span><span class="stock-tag ${stockClass}">${stockLabel}</span>` : ''}
                        </div>
                    </div>
...
function incProduct(pid) {
    ...
    if (!p) return;
    const hasLimit = typeof p.stock === 'number';
    if (hasLimit && p.stock <= 0) return alert(`Maaf, ${p.nama} sedang habis.`);

    let item = cart.find(i => i.pid === pid);
    const currentQty = item ? item.qty : 0;
    if (hasLimit && currentQty + 1 > p.stock) return alert(`Stock ${p.nama} tersisa ${p.stock}. Tidak bisa tambah lagi.`);
