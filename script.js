...
if(resReturnForm) {
    resReturnForm.onsubmit = async (e) => {
        ...
        try {
            ...
            alert("Retur berhasil diajukan! Tiket: " + ticketNum);
            resReturnForm.reset();
...
function loadResellerReturns() {
    db.collection("returns").where("resellerId", "==", currentUser.id).orderBy("createdAt", "desc")
    .onSnapshot(snap => {
...
if(resCompForm) {
    resCompForm.onsubmit = async (e) => {
        ...
        try {
            ...
            alert("Keluhan terkirim! Tiket: " + ticketNum);
            resCompForm.reset();
        } catch (err) { alert(err.message); }
    };
}
...
function loadResellerComplaints() {
    db.collection("complaints").where("resellerId", "==", currentUser.id).orderBy("createdAt", "desc")
    .onSnapshot(snap => {
...
async function loadRankings() {
    try {
        ...
        let ranks = userSnap.docs.map(u => {
            ...
            return { nama: userData.nama, total: total, poin: Math.floor(total / 100) };
        }).sort((a, b) => b.total - a.total);
        const tbody = document.getElementById("adminRankTable");
        if(tbody) {
            tbody.innerHTML = ranks.map((r, i) => `<tr><td align="center">${i+1}</td><td>${r.nama}</td><td align="center">${r.poin.toLocaleString()}</td><td align="right">Rp ${r.total.toLocaleString()}</td></tr>`).join('');
        }
        ...
        if(tbody) {
            tbody.innerHTML = lb.map((res, index) => `
                <tr ${res.id === currentUser.id ? 'style="background:#fff3e0"' : ''}>
                    ...
                    <td>${res.nama} ${res.id === currentUser.id ? '(Saya)' : ''}</td>
                    <td align="center" style="color:#C62828"><b>${res.poin.toLocaleString()}</b></td>
                </tr>`).join('');
        }
...
function loadAdminReturns() {
    db.collection("returns").onSnapshot(snap => {
        ...
        document.getElementById("adminReturnTable").innerHTML = snap.docs.map(d => {
            const r = d.data(); if(r.status==='Proses') p++;
            return `<tr><td>${r.resellerName}</td><td>${r.produk}</td><td>${r.status==='Proses'?`<button onclick="updateStat('returns','${d.id}')" class="btn-gold-sm">Selesai</button>`:'✅'}</td></tr>`;
        }).join('');
        document.getElementById("badgeReturn").innerText = p;
    });
}
...
function loadAdminComplaints() {
    db.collection("complaints").onSnapshot(snap => {
        ...
        document.getElementById("adminCompTable").innerHTML = snap.docs.map(d => {
            const c = d.data(); if(c.status==='Proses') p++;
            return `<tr><td>${c.resellerName}</td><td>${c.keluhan}</td><td>${c.status==='Proses'?`<button onclick="updateStat('complaints','${d.id}')" class="btn-gold-sm">Selesai</button>`:'✅'}</td></tr>`;
        }).join('');
        document.getElementById("badgeComplaint").innerText = p;
    });
}
...
document.getElementById("registerForm").onsubmit = async (e) => { 
    ...
    try { 
        ...
        const cred = await auth.createUserWithEmailAndPassword(email, document.getElementById("regPassword").value); 
...
function loadAdminCatalog() {
    ...
    if(!table) return;
    table.innerHTML = catalog.map(p => `<tr><td>${p.nama}</td><td><small>${p.kategori || '-'}</small></td><td>Rp ${p.harga.toLocaleString()}</td><td><button onclick="prepareEditProduct('${p.id}')" style="color:blue; border:none; background:none;">Edit</button> | <button onclick="deleteProduct('${p.id}')" style="color:red; border:none; background:none;">Hapus</button></td></tr>`).join('');
}
...
if(admProdForm) {
    admProdForm.onsubmit = async (e) => {
        ...
        try {
            ...
            admProdForm.reset(); document.getElementById("prodId").value = "";
        } catch (err) { alert(err.message); }
    };
}
