...
// POIN 2: Load Riwayat untuk Reseller
...
function loadAdminData() {
    ...
    // Update Notif Admin (Retur & Keluhan)
...
// HANDLER RETUR & KELUHAN (RESELLER)
...
document.getElementById("resellerComplaintForm").onsubmit = async (e) => {
    ...
    try {
        await db.collection("complaints").add({
            resellerId: currentUser.id,
            nama: document.getElementById("compNama").value, // POIN 3
            hp: document.getElementById("compHp").value,     // POIN 3
            pesan: document.getElementById("compText").value,
            status: "proses",
            createdAt: firebase.firestore.FieldValue.serverTimestamp()
        });
        alert("Laporan Keluhan Dikirim!");
        e.target.reset();
    } catch(err) { alert(err.message); }
};
