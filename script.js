...
function renderStockListBody(filter = 'semua') {
    ...
    if (!body) return;
    let list = [...catalog];
    if (filter === 'tersedia') list = list.filter(p => typeof p.stock !== 'number' || p.stock > 9);
    if (filter === 'menipis') list = list.filter(p => typeof p.stock === 'number' && p.stock > 0 && p.stock <= 9);
    if (filter === 'habis') list = list.filter(p => typeof p.stock === 'number' && p.stock <= 0);
    if (list.length === 0) {
        body.innerHTML = `<p style="color:#999;">Tidak ada produk untuk kategori ini.</p>`;
        return;
    }
    body.innerHTML = list.map(p => {
        const hasLimit = typeof p.stock === 'number';
        const stock = hasLimit ? p.stock : null;
        const label = !hasLimit ? `<span style="color:#999; font-weight:bold;">Belum diatur</span>` : (stock <= 0 ? `<span style="color:#c0392b; font-weight:bold;">Habis</span>` : (stock <= 9 ? `<span style="color:#c9772a; font-weight:bold;">Menipis</span>` : `<span style="color:#3c6b2a; font-weight:bold;">Tersedia</span>`));
        return `<div style="display:flex; justify-content:space-between; padding:8px 0; border-bottom:1px solid #eee;">
                    <span>${p.nama}</span>
                    <span style="font-size:11px;">${hasLimit ? stock : '-'} — ${label}</span>
                </div>`;
    }).join('');
}
    ...
    db.collection("products").orderBy("kategori").onSnapshot(s => {
        ...
        if (currentUser && currentUser.role === 'admin') {
            document.getElementById("adminCatalogTable").innerHTML = catalog.map(p => {
                const hasLimit = typeof p.stock === 'number';
                const stockLabel = !hasLimit ? `<span style="color:#999;">Belum diatur</span>` : (p.stock <= 0 ? `<span style="color:#c0392b;font-weight:bold;">${p.stock} (Habis)</span>` : (p.stock <= 9 ? `<span style="color:#c9772a;font-weight:bold;">${p.stock} (Menipis)</span>` : `<b>${p.stock}</b>`));
                return `<tr><td><b>${p.nama}</b></td><td>${p.kategori}</td><td>Rp ${p.harga.toLocaleString()}</td><td>${stockLabel}</td>
            ...
            if (admStockEl) admStockEl.innerText = catalog.filter(p => typeof p.stock !== 'number' || p.stock > 0).length;
...
function filterProductsByCategory() {
    ...
    ps.innerHTML = f.map(p => {
        const hasLimit = typeof p.stock === 'number';
        const habis = hasLimit && p.stock <= 0;
        const label = !hasLimit ? '' : (habis ? ' (HABIS)' : ` (Stok: ${p.stock})`);
        return `<option value="${p.id}" ${habis ? 'disabled' : ''}>${p.nama} - Rp${p.harga.toLocaleString()}${label}</option>`;
    }).join('');
}
...
function addToCart() {
    ...
    const hasLimit = typeof p.stock === 'number';
    if (hasLimit) {
        if (p.stock <= 0) return alert(`Maaf, ${p.nama} sedang habis.`);
        const sudahDiKeranjang = cart.filter(i => i.pid === pid).reduce((s, i) => s + i.qty, 0);
        if (sudahDiKeranjang + qty > p.stock) return alert(`Stock ${p.nama} tersisa ${p.stock - sudahDiKeranjang}. Kurangi jumlah pesanan.`);
    }

    cart.push({ pid, nama: p.nama, qty, subtotal: p.harga * qty });
    renderCart();
}
...
document.getElementById("orderFormFinal").onsubmit = async (e) => {
    ...
    try {
        await db.runTransaction(async (t) => {
            ...
            // Cek ulang stock TERBARU (bisa saja berubah sejak dropdown dimuat)
            // Produk yang belum pernah diatur stock-nya (field tidak ada) dianggap tidak dibatasi
            for (let idx = 0; idx < cart.length; idx++) {
                const stockNow = snaps[idx].data()?.stock;
                if (typeof stockNow === 'number' && stockNow < cart[idx].qty) {
                    throw new Error(`Stock ${cart[idx].nama} tinggal ${stockNow}, tidak cukup untuk pesanan ini.`);
                }
            }
            // Potong stock (hanya untuk produk yang field stock-nya sudah diatur)
            snaps.forEach((snap, idx) => {
                const stockNow = snap.data()?.stock;
