...
// --- UPDATE: LOGIKA DATA RESELLER (Statistik & Pesanan dengan Safe Sort) ---
function loadResellerData() {
    ...
    // .orderBy() dihapus agar tidak butuh Composite Index Firestore
    db.collection("orders").where("resellerId", "==", currentUser.id).onSnapshot(sOrders => {
        db.collection("redemptions").where("resellerId", "==", currentUser.id).where("status", "==", "Selesai").onSnapshot(sRedeems => {
            ...
            // Client-Side Sorting: Urutkan data berdasarkan waktu terbaru
            ...
            // Logika Filter Tanggal (Jika input tanggal ada di HTML)
...
// --- UPDATE: RIWAYAT RETUR & KELUHAN (Safe Sort) ---
...
function renderRankTable() {
    ...
    if(document.getElementById("nextRank")) document.getElementById("nextRank").disabled = (currentRankPage >= 4 || endIdx >= allRankings.length);
}
