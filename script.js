...
function initApp() {
    ...
    if (currentUser.role === 'admin') {
        ...
        loadAdminData(); // Fungsi utama admin
    } else {
...
// FUNGSI UTAMA ADMIN (DIPERBAIKI)
function loadAdminData() {
    // 1. Monitor Pesanan Masuk
    ...
    // 2. Monitor Returan Masuk (Poin yang Anda tanyakan)
    db.collection("returns").orderBy("createdAt", "desc").onSnapshot(snap => {
        ...
        if(tb) {
            tb.innerHTML = snap.docs.map(d => {
                const r = d.data();
                return `<tr>
                    <td><b>${r.nama}</b><br><small>${r.hp}</small></td>
                    <td>${r.produk}<br><i style="font-size:10px">${r.alasan}</i></td>
                    <td>${r.status === 'proses' ? `<button onclick="updateStat('returns','${d.id}')" style="background:#C62828; color:white; border:none; padding:5px; border-radius:4px; cursor:pointer;">Selesai</button>` : '✅'}</td>
                </tr>`;
            }).join('');
        }
    });

    // 3. Monitor Keluhan Masuk (Poin yang Anda tanyakan)
    db.collection("complaints").orderBy("createdAt", "desc").onSnapshot(snap => {
        ...
        if(tb) {
            tb.innerHTML = snap.docs.map(d => {
                const c = d.data();
                return `<tr>
                    <td><b>${c.nama}</b><br><small>${c.hp}</small></td>
                    <td>${c.pesan}</td>
                    <td>${c.status === 'proses' ? `<button onclick="updateStat('complaints','${d.id}')" style="background:#C62828; color:white; border:none; padding:5px; border-radius:4px; cursor:pointer;">Selesai</button>` : '✅'}</td>
                </tr>`;
            }).join('');
        }
    });
...
// Fungsi Update Status (Untuk Pesanan, Retur, dan Keluhan)
...
function filterProductsByCategory() {
    ...
    prodSelect.innerHTML = filtered.map(p => `<option value="${p.id}">${p.nama} - Rp${p.harga.toLocaleString()}</option>`).join('');
}
...
async function loadRankings() {
    ...
    if(tbody) {
        tbody.innerHTML = ranks.map((r, i) => `<tr><td>${i+1}</td><td>${r.nama}</td><td>${r.poin}</td><td>Rp ${r.total.toLocaleString()}</td></tr>`).join('');
    }
}
...
async function loadResellerLeaderboard() {
    ...
    if(tbody) {
        tbody.innerHTML = leaderboard.slice(0, 10).map((res, index) => `<tr><td>${index+1}</td><td>${res.nama}</td><td>${res.poin} Poin</td></tr>`).join('');
    }
}
...
function renderCart() {
    ...
    tbody.innerHTML = cart.map((item, index) => { total += item.subtotal; return `<tr><td>${item.nama}</td><td>${item.qty}</td><td>Rp ${item.subtotal.toLocaleString()}</td><td><button onclick="removeFromCart(${index})">X</button></td></tr>`; }).join('');
...
function loadAdminCatalog() {
    ...
    if(!table) return;
    table.innerHTML = catalog.map(p => `<tr><td>${p.nama}</td><td>${p.kategori}</td><td>Rp ${p.harga.toLocaleString()}</td><td><button onclick="prepareEditProduct('${p.id}')">Edit</button></td></tr>`).join('');
}
