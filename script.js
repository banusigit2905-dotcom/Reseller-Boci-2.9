...
function renderStockListBody(filter = 'semua') {
    ...
    if (!body) return;
    let list = [...catalog];
    if (filter === 'tersedia') list = list.filter(p => (p.stock ?? 0) > 9);
    if (filter === 'menipis') list = list.filter(p => (p.stock ?? 0) > 0 && (p.stock ?? 0) <= 9);
    if (filter === 'habis') list = list.filter(p => (p.stock ?? 0) <= 0);
    if (list.length === 0) {
    ...
    body.innerHTML = list.map(p => {
        ...
        const label = stock <= 0 ? `<span style="color:#c0392b; font-weight:bold;">Habis</span>` : (stock <= 9 ? `<span style="color:#c9772a; font-weight:bold;">Menipis</span>` : `<span style="color:#3c6b2a; font-weight:bold;">Tersedia</span>`);
        return `<div style="display:flex; justify-content:space-between; padding:8px 0; border-bottom:1px solid #eee;">
                    <span>${p.nama}</span>
    ...
    db.collection("products").orderBy("kategori").onSnapshot(s => {
        ...
        if (currentUser && currentUser.role === 'admin') {
            document.getElementById("adminCatalogTable").innerHTML = catalog.map(p => {
                const stock = p.stock ?? 0;
                const stockLabel = stock <= 0 ? `<span style="color:#c0392b;font-weight:bold;">${stock} (Habis)</span>` : (stock <= 9 ? `<span style="color:#c9772a;font-weight:bold;">${stock} (Menipis)</span>` : `<b>${stock}</b>`);
            ...
            if (admStockEl) admStockEl.innerText = catalog.filter(p => (p.stock ?? 0) > 0).length;
...
function filterProductsByCategory() {
    ...
    ps.innerHTML = f.map(p => {
        const stock = p.stock ?? 0;
        const habis = stock <= 0;
        return `<option value="${p.id}" ${habis ? 'disabled' : ''}>${p.nama} - Rp${p.harga.toLocaleString()} ${habis ? '(HABIS)' : `(Stok: ${stock})`}</option>`;
    }).join('');
}
...
function addToCart() {
    ...
    if (stock <= 0) return alert(`Maaf, ${p.nama} sedang habis.`);
    if (sudahDiKeranjang + qty > stock) return alert(`Stock ${p.nama} tersisa ${stock - sudahDiKeranjang}. Kurangi jumlah pesanan.`);
    ...
    try {
        await db.runTransaction(async (t) => {
            ...
            for (let idx = 0; idx < cart.length; idx++) {
                const stockNow = snaps[idx].data()?.stock ?? 0;
            ...
            snaps.forEach((snap, idx) => {
                const stockNow = snap.data()?.stock ?? 0;
