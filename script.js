...
function loadAdminData() {
    ...
    db.collection("returns").onSnapshot(snap => {
        ...
        document.getElementById("adminReturnTable").innerHTML = snap.docs.map(d => {
            ...
            return `<tr><td>${d.data().resellerName}</td><td>${d.data().produk}</td><td>${d.data().status==='Proses'?`<button onclick="updateStat('returns','${d.id}')">Selesai</button>`:'✅'}</td></tr>`;
        }).join('');
    ...
    db.collection("complaints").onSnapshot(snap => {
        ...
        document.getElementById("adminCompTable").innerHTML = snap.docs.map(d => {
            if(d.data().status === 'Proses') pending++;
            return `<tr><td>${d.data().pelapor}</td><td>${d.data().isi}</td><td>${d.data().status==='Proses'?`<button onclick="updateStat('complaints','${d.id}')">Selesai</button>`:'✅'}</td></tr>`;
        }).join('');
