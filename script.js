...
// LOGIKA FILTER KATEGORI & PRODUK
function syncCatalog() {
    db.collection("products").onSnapshot(s => {
        catalog = s.docs.map(d => ({ id: d.id, ...d.data() }));
        
        // Update Filter Kategori di Modal
        ...
        filterProductsByCategory(); // Render awal produk
    });
}

function filterProductsByCategory() {
    const selectedCat = document.getElementById("ordCatSelect").value;
...
function loadAdminData() {
    ...
    db.collection("returns").where("status","==","proses").onSnapshot(s => document.getElementById("badgeReturn").innerText = s.size);
    db.collection("complaints").where("status","==","proses").onSnapshot(s => document.getElementById("badgeComplaint").innerText = s.size);
}
...
function openOrderModal() { 
    ...
    renderCart(); 
    goToStep1(); 
    document.getElementById("ordCatSelect").value = "Semua";
